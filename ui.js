function CurrentWeatherUI(currentData) {
	...
	const flag = currentData.sys.country;

	const cityFlag = document.createElement("img");
	cityFlag.setAttribute("class", "city-img");
	cityFlag.setAttribute("src", `https://flagsapi.com/${flag}/flat/24.png`);
	city.appendChild(cityFlag);
	...
	const currAnotherData = document.querySelector(
		".current-anthoer-data"
	).children;
	...
	for (let index = 0; index < arrCurrAnotherData.length; index++) {
		...
		if (index == 2) {
			currAnotherData[2].children[1].innerHTML = `${
				100 - currentData.clouds.all
			}%`;
